...
Only when no matching run exists does it consult the recovery-grade agent-state classifier first, ahead of semantic busy state: a confident dead or missing verdict reports unknown before busy state is even read, closing the case where a harness process died but its pane kept answering as busy.
A terminal `done`/`failed` status-log record is exempt from that verdict, because a recorded outcome is not a liveness claim and must not be withdrawn by a dead harness.
Any other verdict falls through unchanged: exact busy reports working, exact idle permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log.
...
The recovery-grade agent-process liveness classifier is separate from that busy-state poll: tmux and Herdr have verified classifiers used by secondmate recovery, no-run crew-state fallback, and fleet endpoint observations, while Zellij, Orca, and cmux remain unverified.
